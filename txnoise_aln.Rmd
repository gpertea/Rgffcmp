...
```
When splicing noise is added, it looks pretty much the same for multi-exon reads, with STAR sensitivity (recall) trailing in the 70s:
...
ggplot(data=ats, aes(x=as.numeric(factor(sample)), y=num_match_spl/num_sim_spl, group=aligner,
        ...
        scale_x_continuous(breaks=seq(1, 30, 1))
```
When adding the unspliced reads (not just the multi-exon ones) star is catching up, getting raising the recall fraction into the 90s:
...
```
Similarly, the number of reads that should be aligned spliced but instead they were aligned without introns (missed spliced alignments!):
