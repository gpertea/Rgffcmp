...
library(plotly)
# load a tmap file
tmapZ<-read.table('data/riboZ/cmpR3907.R3907_C3RGLACXX.gtf.tmap', 
               header=T, sep='\t')
gtitleZ <- "R3907 riboZ"

tmapZ<-read.table('data/riboZ/cmpR3907.R3907_C3RGLACXX.gtf.tmap', 
                  header=T, sep='\t')
tmapZ<-subset(tmapZ, FPKM>1.0 & num_exons>1, 
              select=c(ref_id,class_code,qry_id, num_exons, FPKM, TPM, len))
gtitleZ <- "R3907 riboZ"

tmapZ$prot <- gtitleZ

tmapA<-read.table('data/polyA/cmpR3907.R3907_C101EACXX.gtf.tmap', 
                  header=T, sep='\t')
tmapA<-subset(tmapA, FPKM>1.0 & num_exons>1, 
              select=c(ref_id,class_code,qry_id, num_exons, FPKM, TPM, len))
gtitleA <- "R3907 polyA"

tmapA$prot <- gtitleA

tmap <- rbind(tmapA, tmapZ) 
